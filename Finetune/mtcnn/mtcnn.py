...
from .utils.detect_face import detect_face, extract_face, save_landmark
...
class MTCNN(nn.Module):
    ...
    def forward(self, img, save_path=None, return_prob=False, save_landmarks=False):
        ...
        # Detect faces
        with torch.no_grad():
            res = self.detect(img, save_landmarks)
        if save_landmarks:
            batch_boxes, batch_probs, batch_landmarks = res[0], res[1], res[2]
        else:
            batch_boxes, batch_probs = res[0], res[1]
        ...
        faces, probs = [], []
        for idx, (im, box_im, prob_im, path_im) in enumerate(zip(img, batch_boxes, batch_probs, save_path)):
            if box_im is None:
                faces.append(None)
            ...
            for i, box in enumerate(box_im):
                ...
                if path_im is not None and i > 0:
                    ...
                    landmark_path = save_name + '_' + str(i + 1) + '.txt'

                face = extract_face(im, box, self.image_size,
                                    self.margin, face_path)
                if save_landmarks:
                    save_landmark(im, box, self.image_size, self.margin, land, landmark_path)
